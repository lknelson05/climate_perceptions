...
```{r load data}
responses <- read_excel("../data/responses_9.24.2020.xlsx")
```
...
```{r adding broader fishery category}
#this might be for the Northeast only because we broke out all groundfish species
responses$groundfish_category = rep(NA, length =  nrow(responses))
...
for(i in 1: nrow(responses)) {
  ...
  if(responses$cod[i] == 1) {
    if(fisheriesname == "") { # have to do extra to not put a , at the beginning
      ...
      fisheriesname = paste(fisheriesname, "groundfish")
    } else {
  ...
  responses$groundfish_category[i] = fisheriesname
}
...
responses <- responses %>% 
  rename(crew_num = q9) %>% 
  mutate(crew_num = case_when(crew_num == 1 ~ "0",
                              crew_num == 2 ~ "1-3",
                              crew_num == 3 ~ "4-7",
                              crew_num == 4 ~ "7-11",
                              crew_num == 5 ~ "11+"))
...
```{r question 12}
#Do you currently participate in fisheries in other parts of the country in addition to those off ME, NH, MA, RI, or CT?
...
responses <- responses %>% 
  rename(income = q14) %>% 
  mutate(income = case_when(income == 1 ~ "0", #was none
                            income == 2 ~ "U10",
                            income == 3 ~ "10-25",
                            income == 4 ~ "25-50",
                            income == 5 ~ "50+"))
...
#questions 18 and 19 are about drinking and smoking and appear in different places in the survey versions. Chunks to recode those questions are near the end of this file.
```{r question 20}
...
```{r question 33/18}
#this is a repeat 33 it does come after 36a and b, this is question 18 in the PNW survey
...
```{r question 34/19}
#this is question 19 in the PNW survey
...
#330 = MA
responses[330, 290] = "Massachusetts"
responses[330, 291] = "MA"

#370 = ME
responses[370, 290] = "Maine"
responses[370, 291] = "ME"
```

#After saving I cleaned up responses for ports, other fisheries, other fishing regions, other gear types. 
```{r save data}
