...
responses <- responses %>%
  mutate(typology = case_when(port_clean == "Astoria" ~ "cfts",
         port_clean =="Brookings" ~ "cfts",
         port_clean =="Chinook" ~ "cfts",
...
num_fisheries <- ggplot(responses, aes(x=num_fisheries, fill = typology))+
...
ggsave(plot = income, file = paste0("../figures/OMF/pct income outside fishing.png"))
...
fish_elsewhere <- ggplot(responses, aes(x=other_fisheries, fill = typology))+
  geom_bar(position = position_dodge(), stat="count")+
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1))+ ylab("Number of Respondents\n")+
  scale_fill_manual(values = pal4) +
  #xlab("Number of fisheries\n")+
...
ggsave(plot = fish_elsewhere, file = paste0("../figures/OMF/fish other places.png"))
...
fish_climate <- ggplot(responses, aes(x=ability, fill = typology))+
  geom_bar(position = position_dodge(), stat="count")+
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1))+ ylab("Number of Respondents\n")+
  scale_fill_manual(values = pal4) +
...
ggsave(plot = fish_climate, file = paste0("../figures/OMF/fishing affected by climate.png"))
