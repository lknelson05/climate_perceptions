...
```{r}
indicator_corr<-ggcorr(ac[,1:24])

indicator_corr

indicator_cor<-cor(ac[,1:24], method = "pearson")
indicator_cor

cp<-cor.plot(ac[,1:24], numbers = TRUE)
cp
...
ages<-full_join(ages_wc, ages_bc)
ages<-full_join(ages, ages_ne)

domainsmod<-left_join(domainsmod, ages)
...
#consolidate age groups to over and under 60
