...
state <- responses%>%
  ...
  #rename(state_code = port_state_code)
...
#the loop makes it a character for some reason
#state_ne$home_zip<-as.factor(state_ne$home_zip)
...
```{r}
#anova home state
hs_aov<-aov(ac ~ state_code, data = domainsmod)
summary(hs_aov)
...
#box plots
domainsmod$state_code<-as.factor(domainsmod$state_code)

state_ac<-domainsmod %>%
  select(state_code, ac)

state_box_plot<-ggplot(state_ac, aes(x=state_code, y = ac, fill = state_code)) + geom_boxplot()+
  #facet_wrap(~state_code, scale = "free")+
  scale_fill_manual(values = pnw_palette(n=12, name = "Bay", type = "continuous"))

state_box_plot

ggsave(plot = state_box_plot, height = 7, width = 10, file = paste0("../figures/state ac boxplot.png"))
```
...
us_counties <- counties("US", year = 2020)%>%
  shift_geometry()


tm_shape(us_counties)+ tm_polygons(col="grey", border.col="white")
