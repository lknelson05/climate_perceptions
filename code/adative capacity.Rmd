...
```{r}
ac_barnes<-responses%>%
  select(money, vessel_owner, loan, income, other_fisheries, yrs_fishing, effective_mgmt, equitable, adapt_mgmt, connect_env, connect_comm, strong_comm, generation, believe, harm_me, harm_future, voice, const_reg, no_point) 
...
```{r}
indicator_corr<-ggcorr(ac_barnes[,1:18])

indicator_corr

indicator_cor<-cor(ac_barnes[,1:18], method = "pearson")
...
new_ac_pca<-prcomp(ac_barnes[,1:18], scale. = FALSE)
summary(new_ac_pca)
