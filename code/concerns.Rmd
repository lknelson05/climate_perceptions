...
```{r load data}
responses <- read.csv("../data/PNW_decoded_responses_29.Apr.2021.csv")
```
#concerns table
...
#pca of concern answers using my rescale
```{r}
colnames(concern_scores_rescaled)<-c("environment", "fishing_ops", "community_and_infrastructure", "personal", "outlook", "conflict", "groups")

concerns_pca<-prcomp(concern_scores_rescaled[,1:6], scale = FALSE)
...
ggsave(plot = concerns_scree, file = paste0("../figures/wellbeing/concern_scree3.png"))

ggsave(plot = concerns_variables, file = paste0("../figures/wellbeing/concern_variable3.png"))
```

##pca using the scale function of prcomp
```{r}

test.concern.pca<-prcomp(concern_scores, scale = TRUE)
summary(test.concern.pca)
summary(concerns_pca)

#eigenvalues
eig_concern_test<-get_eig(test.concern.pca)
eig_concern_test

#results for variable
var_concern_test <- get_pca_var(test.concern.pca)
var_concern$coord          # Coordinates
var_concern$contrib        # Contributions to the PCs
var_concern$cos2           # Quality of representation 

pal<-pnw_palette(name="Bay", n=3, type = "discrete")
concerns_pca_test_plot<-ggbiplot(test.concern.pca, ellipse = TRUE, groups = concern_scores_rescaled$groups) +
  scale_color_manual(values = pnw_palette(n=3, name = "Bay"))+
  theme(legend.position = "bottom")

concerns_pca_test_plot

ggsave(plot = concerns_pca_plot, file = paste0("../figures/wellbeing/concern_pca.png"))

#screeplot of components
concerns_scree_test<-fviz_eig(test.concern.pca)
concerns_scree_test

concerns_variables_test<-fviz_pca_var(test.concern.pca, repel = TRUE)
concerns_variables_test
...
#marine environment
```{r}
en.av<-aov(environment ~ groups, data = concern_scores_rescaled)
summary(en.av)
...
oneway.test(environment ~ groups, data = concern_scores_rescaled)

pairwise.t.test(concern_scores_rescaled$environment, concern_scores_rescaled$groups,
                 p.adjust.method = "BH", pool.sd = FALSE)
```
##fishing operations
```{r}
fo.av<-aov(fishing_ops ~ groups, data = concern_scores_rescaled)
summary(fo.av)
...
#homogenity of variance
#levene test for homogenity of variance
leveneTest(fishing_ops ~ groups, data = concern_scores_rescaled)
...
oneway.test(fishing_ops ~ groups, data = concern_scores_rescaled)

pairwise.t.test(concern_scores_rescaled$fishing_ops, concern_scores_rescaled$groups,
                 p.adjust.method = "BH", pool.sd = FALSE)
```
##community and infrastructure
```{r}
cai.av<-aov(community_and_infrastructure ~ groups, data = concern_scores_rescaled)
summary(cai.av)
...
#homogenity of variance
#levene test for homogenity of variance
leveneTest(community_and_infrastructure ~ groups, data = concern_scores_rescaled)
...
oneway.test(community_and_infrastructure ~ groups, data = concern_scores_rescaled)

pairwise.t.test(concern_scores_rescaled$community_and_infrastructure, concern_scores_rescaled$groups,
                 p.adjust.method = "BH", pool.sd = FALSE)
```

##personal
```{r}
per.av<-aov(personal ~ groups, data = concern_scores_rescaled)
summary(per.av)
...
oneway.test(personal ~ groups, data = concern_scores_rescaled)

pairwise.t.test(concern_scores_rescaled$personal, concern_scores_rescaled$groups,
                 p.adjust.method = "BH", pool.sd = FALSE)
```

##conflict
```{r}
con.av<-aov(conflict ~ groups, data = concern_scores_rescaled)
summary(con.av)
...
oneway.test(conflict ~ groups, data = concern_scores_rescaled)

pairwise.t.test(concern_scores_rescaled$conflict, concern_scores_rescaled$groups,
                 p.adjust.method = "BH", pool.sd = FALSE)

kruskal.test(conflict ~ groups, data = concern_scores_rescaled)
```

##outlook
```{r}
out.av<-aov(outlook ~ groups, data = concern_scores_rescaled)
summary(out.av)
...
oneway.test(outlook ~ groups, data = concern_scores_rescaled)

pairwise.t.test(concern_scores_rescaled$outlook, concern_scores_rescaled$groups,
                 p.adjust.method = "BH", pool.sd = FALSE)

kruskal.test(outlook ~ groups, data = concern_scores_rescaled)
```
```
