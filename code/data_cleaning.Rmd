...
```{r load data}
responses <- read_excel("../data/responses_7.10.2020.xlsx")
```
...
responses <- responses %>% 
  unlist(responses$fish_neg_wellbeing)  %>%  
  unlist(responses$fish_neg_health) %>% 
  unlist(responses$fish_neg_mental)  %>% 
  unlist(responses$fish_increase_stress) %>%  
  unlist(responses$env_neg_welbeing)  %>% 
  unlist(responses$env_neg_health)  %>% 
  unlist(responses$env_neg_mental)  %>% 
  unlist(responses$env_increase_stress) %>% 
  unlist(responses$env_neg_safety)
```
...
write.csv(responses, paste0("../data/decoded_responses", format(Sys.time(), "%d.%b.%Y"),".csv"))
```
