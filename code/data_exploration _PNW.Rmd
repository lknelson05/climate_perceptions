...
```{r load data}
responses <- read.csv("../data/decoded_responses_08.Oct.2020.csv")
...
#responses by fishing area
```{r responses by fishing area}
#update based upon survey region
responses$fishing_area <- factor(responses$fishing_area, levels = c("Washington", "Oregon", "California"))

responses_by_fishing_area_plot <- ggplot(data = responses, aes(x=state))+
  geom_bar(stat = "count")+
  xlab("State\n")+
  ggtitle("Respondents by State") 
