...
```{r load data}
responses <- read.csv("../data/decoded_responses16.Jul.2020.csv")
...
responses <- responses %>% 
  # select(homeport, port_zip, hometown, home_zip) %>% 
  left_join(zips)
...
responses$state <- factor(responses$state, levels = c("New York", "Connecticut", "Rhode Island", "Massachusetts", "New Hampshire", "Maine"))
...
ggsave(plot = cc_belief_plot, file = paste0("../figures/response_by_state.png"))
...
belief <- responses %>% 
  select(age, state, believe, harm_me, harm_future)
...
ggsave(plot = cc_belief_plot, file = paste0("../figures/climate_change_beliefe_age.png"))
