...
responses <- read.csv("../data/decoded_responses_01.Jun.2021.csv")
```
...
```{r zipcode data, eval=FALSE, include=FALSE}
...
```{r add state and port data to responses, eval=FALSE, include=FALSE}
...
```{r responses by State, eval=FALSE, include=FALSE}
...
pal7<-pnw_palette("Sailboat", 7, type = "continuous")
```
...
regions <-  responses %>%
  select(offshore, Haida_Gwaii, ncoast, cencoast, NVI, WCVI, SG, SJF, IW, USW) %>%  #other_region is also an option
  gather(region, response) %>% 
  group_by(region)
regions$region <- factor(regions$region, levels = c("offshore", "Haida_Gwaii", "ncoast", "cencoast", "NVI", "WCVI", "SG", "SJF", "IW", "USW"))
region_plot <- ggplot()+
...
job <-  responses %>%
  select(licenseowner, captain, vesselowner, crew, other_role) %>%  
  gather(role, response) %>% 
  group_by(role)
job$role <- factor(job$role, levels = c("licenseowner", "captain", "vesselowner", "crew", "other_role"))
job_plot <- ggplot()+
...
responses$vessel <- factor(responses$vessel, levels = c("U25", "25-35", "36-45", "46-55", "56-65", "66-85", "86+"))
...
vessel_pct <- ggplot(data = responses, aes(x=vessel))+
  geom_bar(aes(y = (..count..)/sum(..count..)), fill = pal7)+
  scale_y_continuous(labels=scales::percent) +
  xlab("vessel length\n")+ ylab("percent") +
...
for(i in 1: nrow(responses)) {
  inrole = ""
  if(responses$licenseowner[i] == 1) {
    inrole = paste(inrole, "license owner", sep = "") 
  }
...
responses_income_plot <- ggplot(data = responses, aes(x=income))+
  geom_bar(fill = "#749dae", stat = "count")+
...
belief <- responses %>% 
  select(age, ability, believe, harm_me, harm_future)
...
```{r belief in climate change by State, eval=FALSE, include=FALSE}
...
```{r climate change harm me - state, eval=FALSE, include=FALSE}
...
```{r climate change harm future - state, eval=FALSE, include=FALSE}
