...
##Beginning to investigate the survey data with some basic summary stats and figures
##Figures are saved to ../figures/resp_sum/ (respondant summary)
##To switch between PNW/BC/NE surveys, fisheries, gear types, and geography needs to be updated to the appropriate set of options
##Saves data at the end with state info attached
...
zips<- ZipCodes %>% 
  select(home_zip, state, state_code, County) %>% 
...
names(portzips) = c("port_zip", "port_state", "port_state_code", "port_county")
...
#fix a few of the portzips where there was a typo
responses <- responses %>%
  mutate(port_zip=ifelse(port_zip=="98365", "97365",
                         port_zip))

responses$port_zip <- as.integer(responses$port_zip)
...
obs$ocean_temp <- factor(obs$ocean_temp, levels = c("no change", "increase", "decrease"))
...
obs$severe_weather <- factor(obs$severe_weather, levels = c("no change", "increase", "decrease"))
...
obs$target_spp <- factor(obs$target_spp, levels = c("no change", "increase", "decrease"))
...
belief<-belief %>%
  mutate(ability = case_when(
    (ability == "no" ~ "no"),
    (ability == "no_obs_change" ~ "no observations of change"),
...
cc_ability_plot <- ggplot(data = belief, aes (x=ability))+
  geom_bar(aes(y = (..count..)/sum(..count..)), fill = pal)+
  scale_y_continuous(labels=scales::percent) +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1))+ ylab("percent\n")+
  xlab("Category\n")+
