...
ps<-domains<-domains %>%
  filter(region == "esjf_ps")

stand<-sd(ps$ac)

outliervalue <- mean(ps$ac) - 2.5*stand
```
...
```{r}
rcomp<-domains %>%
  select(assets, flexibility, organization, learning, sociocog, agency, region)
...
rownames(rsum)<-rsum$region

bb<-predict(ac_dom_pca, newdata = rsum)

fviz_add(dom_pca_plot, bb, color = "navy", repel = TRUE)
```
...
```
```{r}
table(domainsmod$Commercial.Reliance.Categorical.Ranking, responses$other_job)
